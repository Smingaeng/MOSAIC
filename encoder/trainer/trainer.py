...
class Trainer(object):
    ...
    def create_optimizer(self, model):
        ...
        if optim_config['name'] == 'adam':
            ...
            if hasattr(model, 'get_optimizer_param_groups'):
                params = model.get_optimizer_param_groups(optim_config['lr'])
            self.optimizer = optim.Adam(params, lr=optim_config['lr'], weight_decay=optim_config['weight_decay'])
    ...
    def train_epoch(self, model, epoch_idx):
        ...
        # start this epoch
        model.train()
        show_progress = configs['train'].get('show_progress', True)
        for _, tem in tqdm(
            enumerate(train_dataloader),
            desc='Training Recommender',
    ...
    def train(self, model):
        ...
        # test_result_2 = self.sparse_test(model, self.data_handler.test_dataloader_2)
        # self.logger.log("Best Epoch {}. Final test result: {}.".format(best_epoch, test_result_2))
        # test_result_3 = self.sparse_test(model, self.data_handler.test_dataloader_3)
        # self.logger.log("Best Epoch {}. Final test result: {}.".format(best_epoch, test_result_3))
        # test_result_4 = self.sparse_test(model, self.data_handler.test_dataloader_4)
        # self.logger.log("Best Epoch {}. Final test result: {}.".format(best_epoch, test_result_4))
    ...
    # @log_exceptions
    # def sparse_test(self, model, data):
    #     model.eval()
    #     eval_result = self.metric.eval(model, data)
    #     self.logger.log_eval(eval_result, configs['test']['k'], data_type='Test set')
    #     return eval_result
    ...
    def save_model(self, model):
        if configs['train']['save_model']:
            ...
            model_name = configs['model'].get('run_name', configs['model']['name'])
