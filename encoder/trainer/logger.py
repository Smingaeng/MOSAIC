...
def get_local_time():
    return datetime.datetime.now().strftime('%b-%d-%Y_%H-%M-%S_%f')
...
class Logger(object):
    def __init__(self, log_configs=True):
        ...
        logger_name = 'train_logger_{}_{}_{}_pid{}'.format(
            model_name,
            dataset_name,
            get_local_time(),
            os.getpid(),
        ...
        if not configs['tune']['enable']:
            log_file = logging.FileHandler(
                '{}/{}_{}_pid{}.log'.format(log_dir_path, dataset_name, get_local_time(), os.getpid()),
                'a',
                encoding='utf-8',
            )
        else:
            log_file = logging.FileHandler(
                '{}/{}-tune_{}_pid{}.log'.format(log_dir_path, dataset_name, get_local_time(), os.getpid()),
                'a',
                encoding='utf-8',
            )
